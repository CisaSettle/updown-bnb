...
对每个市场，keeper 会先读取 `maintenanceRequired()`。如果没有任何有资金的轮次仍需锁定或结算，它就按
`IDLE_POLL_MS` 休眠：空的时间网格轮次保持虚拟开盘，不消耗 gas。

如果旧部署还没有这个 selector，keeper 会自动保留旧版常开调度，因此可以先更新 keeper，也可以先切换六个新地址，
不会在滚动发布期间漏掉旧仓位。测试网的空盘只会在距离锁盘至少 50 秒时接收首注。这个上限按最坏配置路径计算：
1 秒空盘轮询 + 两个各 4 秒的报价源 + 三个各 12 秒的中继队列槽 + 5 秒快照 / 模拟 RPC 余量 = 50 秒。任何超过
合约截止线的配置都会在 keeper 启动时被拒绝。下一个网格轮次无需交易也会自行开盘。
...
| `IDLE_POLL_MS` | `1000` | 空盘时只做一次廉价的 `maintenanceRequired()` 读取，足以在 1 分钟测试网边界前捕捉首笔下注；也用于重新轮询已暂停 / 尚未启动的市场 |
...
| `inactive` | 是 | 当前没有任何有资金的轮次需要 keeper 交易。包括空轮次仍在虚拟开盘的市场，以及尚未调用 `genesisStart()` 的市场。**暂停是另一个独立状态**，不是这个 | 无 |
| `degraded` | **否** | keeper 按时在调用，但它结算出来的东西是无价值的 —— 或者它根本没有调用，而 `stale` 看不见。四种成因：keeper 侧故障（某个中继喂价不接受这把私钥）；近期结算窗口内故障作废次数过多；**连续两个有资金的活跃时段过去而本 keeper 一次都没有执行**（有资金的时段在 `lockTs + bufferSeconds` 结束，短于两个 interval 的预算，而每次唤醒都会重置该预算，所以在下注时断时续的市场上，一个从不执行的 keeper 永远不会变成 `stale`；只有链上时间已过该截止点才计一次，所以滞后 RPC 节点的一次回答不会被算进去）；或者**一个已锁定轮次的结算窗口在暂停期间耗尽的暂停市场**，这刚刚把一个已定的结果变成了退款 | **呼叫值班。** 对错过活跃时段这一成因：先解决 `executeRound` 发不出去的原因（gas、RPC、私钥 —— §3.1），之后该状态会在下一次成功执行、keeper 重启（计数只在进程内）或最后一次错过之后六小时时清除。对第四种成因，解除暂停**不是**解法，结算才是，而且它在暂停状态下照样可用。见 §3.4 与 §4 的残余风险 2 |
...
`warnings[]` 承载非致命状况，主要是 **keeper 余额过低**。当所有市场都是空盘时，没有交易要发，keeper 没 gas
只是告警；一旦有真实资金需要锁定或结算，它就升级为 blocker，`/healthz` 返回 503。
...
市场集合不是当前六个 1 分钟/10 分钟市场；某个市场在 `/healthz` 上报的地址与 `DEPLOYMENTS_PATH` 中的不一致（仍在
服务已被替换部署的 keeper 进程，上报的名称和绿色状态与新部署完全相同）；RPC 不是链 97；keeper、bot 或 funder 的
gas 穿过配置阈值。最后一项专门捕获本次静默状态：市场仍然开放，但两个演示流动性账户都已无法落下唤醒轮次的第一注。
早于地址上报功能的旧版 keeper 无法用这种方式核对：看门狗每次运行都会记录这一点，并在它未经核对地运行超过
`UPDOWN_UNVERIFIED_GRACE_SECONDS`（默认 600）后判定失败——这段宽限期覆盖了 rsync dist 到 keeper 重启之间的那一分钟，
不会误报。若某一轮连 `/healthz` 都读不到、或部署清单读不出来，则原样保留这个计时，避免端点抖动把未核对的 keeper
永远压在宽限期内。

当**任何一个市场**都已超过 `UPDOWN_MARKET_MAKING_MAX_IDLE_SECONDS`（默认 3600，填 0 关闭）没有承接过内部下注时，
它同样判定失败——这个时间直接从链上的下注账本读取。2026-09-04 那次正是靠它才能被发现：下注机器人死了 20.7 小时，
而这只看门狗连续 1976 次运行全部报「healthy」，因为死掉的机器人不花 gas，余额只会离下限越来越**远**，它也不结算，
所以 `/healthz` 一直是绿的。这个检查刻意是全盘面而非逐市场的——为了省 gas，机器人经常被收窄到只做一个市场，
五个市场安静是设计中的常态，只有全部同时沉默才说明进程没了。如果所有账户在所有市场上**从来没有**下注过，同样判失败，
而不是当作「没测到」：那正是全新部署上机器人压根没启动起来的样子。
...
报告里还带着一件它自己做不了的事。`【tBNB 领取（人工，发送时）】` 一节写明领取地址、水龙头链接、可动用余额、实测日耗和
预计续航天数，最后给出 `✅ 今天不用领` 或 `🔴 今天去领一次`。无论当天是否需要领，它都会出现——它的用途是省掉一次查找，而不是报警。

其中有两处是承重的。地址是 **gas 加注账户**，而且只有它能用：水龙头只服务那些已经在 BSC 主网上持币的地址，而项目的五个运营账户里
只有它满足（主网 0.031 BNB，门槛 0.002，由一次只读的 `eth_getBalance` 现场核对——这个进程在任何链上都不签名）。一旦这个余额跌破门槛，
这一行会转红并直说水龙头会拒绝；否则那就变成浏览器里一句莫名其妙的拒绝。日耗是**测出来的，不是假设的**：每次运行都会把运营账户的总余额
写进状态文件，下一次运行用差值除以间隔时间，所以续航跟着真实的回合节奏、市场集合和 gas 价格走，而不是一个曾经成立过的常数。在跑满两次
之前它显示 `暂无`，刚补过币之后也一样——余额变多是「这次没测到」，不是负的消耗。

水龙头本身也会空，而这一节会赶在浏览器之前说出来。官方水龙头 24 小时放行一次，除此之外还要收币地址在
BSC 主网上持有 0.002 BNB——项目里只有 gas 加注账户满足；但 2026-09-04 那天，一个满足门槛、验证码也过了的地址拿到的回答是
`The faucet has insufficient funds`。至于每次给多少，这里刻意不写：理由见下面 §2——那个数字是外部的，会变。所以官方链接下面还跟着一行 `备用水龙头`，内容就是 `UPDOWN_FAUCET_FALLBACK_URLS`
里的清单：默认是 QuickNode（<https://faucet.quicknode.com/binance-smart-chain>，不要求主网余额、也不要求注册账号，12
小时一次）和 Bitbond（<https://tokentool.bitbond.com/faucet/bsc-testnet>，要求连接钱包并补全个人资料，24 小时一次）。
把这份清单配成空，这一行就不再出现。它写的是每一家**要求什么**，而不是给多少：额度与政策都是外部条件，说变就变——和下面
补给流程里对官方水龙头的那句提醒是同一件事。它们领到的都是同一个地址，所以主网资格只对会检查它的那一家有意义，
换一家领并不会改变 tBNB 的落点。
...
✅ 无未按时结算的退款，而 数据健康 一节会重新核对每个市场持有的 USDT 至少等于 `outstanding + treasuryAmount`。

某个市场一个回合都没开出来，永远不算故障，哪怕其余市场照常成交。时间格只有在有人下注时才会变成回合——这正是
`_activateBettableRound` 做的事，而 keeper 只结算下注撑起来的那些——所以回合为零永远等于需求为零；keeper 卡住会以另一种方式暴露：
有资金的回合没能按时结算。这一点在实践中很重要：测试网 gas 紧张期间，演示流动性被刻意限制在三个 10m 市场上，1m 市场本来就应该是空的。
它们会以 `无成交` 一行列在 各市场 之下，而不是从表里消失——因为行没了之后，「消失的行」和「没人碰过的市场」看起来一模一样。
...
演示流动性机器人每轮都下注时，按当前 1 分钟节奏，整盘每天大约烧掉 **0.25–0.30 tBNB**；实际数字会随 BSC
测试网 gas 价格和机器人领取历史仓位的次数波动。没有下注时，空轮次是虚拟的，keeper/中继的持续 gas 成本为零。
tBNB 只有链自己的水龙头能铸，所以只要启用演示流动性或存在真实资金仓位，它仍是运行依赖。

先盯有资金的风险。机器人断气只是盘口不动，但空盘口仍正常开盘。keeper 没 gas 只有在有人下注后才会影响该轮：
有资金的轮次可能耗尽锁定/结算窗口并退款。`MIN_BALANCE_BNB` 告警（默认 0.05）会提前出现，但不会把空的虚拟市场
判成停盘。

**`RELAY_TICK_MS` 就是那个杠杆，而设成 `0` 是完全没有代价的那一档。** 推价的其实是两件不同的事，只有其中一件跟钱有关：每一个边界时刻都有自己那笔**必需**的中继报价，结算完全依赖它；而
`RELAY_TICK_MS`（这里是 30 秒，默认关闭）买的是边界之间的**额外**报价，作用只有让价格图更密——一旦有边界中继要发，密度推送是被跳过而不是排队，它也从不占用
`(feed, boundary)` 的认领。关掉它只会让价格图变粗，同时削掉 keeper 相当一部分 gas。

结算**不依赖**密度推送，但开着它也并非全无影响：两者共用同一把私钥、因而共用同一个 nonce，所以一笔已广播却始终没确认的密度推送，会挡在下一笔边界中继前面直到它清掉——keeper
...
```bash
SRC_KEY=0x… BOT_ADDRESSES=<botA>,<botB> node scripts/fund-gas.mjs --dry   # 确认无误后去掉 --dry
```

`fund-gas.mjs` 在签任何东西之前会先确认 RPC 回的是链 97：出资私钥在同一个地址上控制着主网的真钱，连错端点就是把它花掉。它是「补到目标值」而不是「发固定金额」，
并且在源账户不够时按同一个比例缩放每一笔转账——这样一次不完整的领取会让整盘落在同一个到期时间上，而不是把前面的账户填满、把最后一个饿死。
