...
test('reads the projected bettable epoch instead of the stale materialized epoch', async () => {
  ...
  assert.deepEqual(result, { epoch: 153n, round: projected, firstBetMinLeadSeconds: 50n })
...
test('requires the full dormant-relay runway for the first stake', () => {
  ...
  assert.equal(hasPlanningRunway(empty, 1_009, 50n), true)
  assert.equal(hasPlanningRunway(empty, 1_010, 50n), false)
})
...
test('keeps the shorter planning cushion once a round is funded', () => {
  ...
  assert.equal(hasPlanningRunway(funded, 1_039, 50n), true)
  assert.equal(hasPlanningRunway(funded, 1_040, 50n), false)
})
